...
function ShoppingListItem({id, quantity, shoppingList, addToShoppingList }){

    const { isOpen, onOpen, onClose } = useDisclosure();

    function setQuantity(quantity){
        const new_shopping_list = [...shoppingList];
        const index = new_shopping_list.findIndex((p) => p.id === id);
        new_shopping_list[index].quantity = quantity
